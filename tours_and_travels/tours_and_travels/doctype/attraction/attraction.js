...
frappe.ui.form.on('Attraction', {
    attraction_address: function(frm) {
        if (frm.doc.attraction_address) {
            return frm.call({
                method: "frappe.contacts.doctype.address.address.get_address_display",
                args: {
                    "address_dict": frm.doc.attraction_address
                },
                callback: function(r) {
                    if(r.message)
    ...
    refresh: function(frm) {
        if (frm.doc.attraction_address && !frm.doc.address_display) {
            return frm.call({
                method: "frappe.contacts.doctype.address.address.get_address_display",
                args: {
                    "address_dict": frm.doc.attraction_address
        ...
        frm.set_query('attraction_address', function(doc) {
            return {
                filters: {
                    'link_doctype': 'Attraction',
...
frappe.ui.form.on('Attraction Service List', {
    attraction_service: function(frm, cdt, cdn) {
        var d = locals[cdt][cdn];
        $.each(frm.doc.attraction_service_list, function(i, row) {
            if (row.attraction_service === d.attraction_service && row.name != d.name) {
                frappe.msgprint('The '+ '<b>' + d.attraction_service + '</b>' +' you added already exists on the table.');
                frappe.model.remove_from_locals(cdt, cdn);
                frm.refresh_field('attraction_service_list');
                return false;
            }
        });
    }
